...
export default function SignUpPage() {
  ...
  return (
    <AuthShell
      ...
      subtitle="Join Zimoji and start sharing your moods."
      footer={
        <>
          Already have an account?{" "}
          <Link href="/signin" className="font-semibold grad-text">
            Sign in
          </Link>
        </>
      }
    >
      <form onSubmit={(e) => e.preventDefault()} className="space-y-4">
        ...
        {/* Pick your starting mood */}
        <div>
          <p className="mb-1.5 text-[13px] font-semibold text-[var(--ink-700)]">Your vibe today</p>
            ...
            {moods.map((m) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-sm border px-2.5 py-1.5 text-[12.5px] font-semibold transition-colors"
                  style={{
                    background: active ? m.chip : "white",
                    color: active ? m.accent : "var(--ink-500)",
                    borderColor: active ? m.border : "var(--line)",
                  }}
                >
        ...
        <label className="flex select-none items-start gap-2 text-[12.5px] text-[var(--ink-500)]">
          <input type="checkbox" className="mt-0.5 h-4 w-4 rounded accent-[var(--brand-600)]" />
          <span>
            I agree to Zimoji&apos;s{" "}
            <Link href="#" className="font-semibold text-[var(--brand-600)] hover:underline">Terms</Link> and{" "}
            <Link href="#" className="font-semibold text-[var(--brand-600)] hover:underline">Privacy Policy</Link>.
          </span>
        </label>
